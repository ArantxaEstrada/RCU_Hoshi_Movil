...
export default function UTecnico({ navigation, route }) {
    ...
    useEffect(() => {
        if (tecnico) {
            setNombre(tecnico.usr_nombre || '');
            setApellidoPaterno(tecnico.usr_ap || '');
            setApellidoMaterno(tecnico.usr_am || '');
            setCorreo(tecnico.usr_correo || '');
            setPassword(tecnico.usr_pass || '');
            setEstado(tecnico.est_tipo || 1);
        }
    }, [tecnico]);
    const handleUpdate = async () => {
        try {
            ...
            if (correoLower !== tecnico.usr_correo.toLowerCase()) {
            ...
            if (updateError) {
                setError(updateError.message || 'No se pudo actualizar el técnico.');
                setLoading(false);
                return;
            }
            ...
            setError('');
            setLoading(false);
            setConfirm('Técnico actualizado exitosamente');
        } catch (err) {
    ...
    if (data) {
        if (data.est_tipo != 2 && data.perf_tipo === 1) {
            ...
            if (!tecnico) {
                navigation.replace('rtecnico', { data });
                return null;
            }
            ...
            return (
                <SafeAreaView style={styles.safe}>
                    ...
                    <View style={styles.loggedInContainer2}>
                        <Text style={styles.welcomeText}>Actualizar técnico</Text>
                        <View style={styles.formCard2}>
                            <ScrollView style={styles.formCardScroll} showsVerticalScrollIndicator={false}>
                                <Text style={styles.label2}>Boleta del técnico</Text>
                                ...
                                <Text style={styles.label2}>Estado del técnico</Text>
                                <TouchableOpacity
                                    style={styles.input2}
